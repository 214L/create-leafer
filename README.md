...
| 命令 | 用途 |
| --- | --- |
| `template` | 创建一个新的 Leafer 模版项目 |
| `init` | 在已有项目中引入 Leafer 依赖 |
| `add` | 添加或更新项目中的 Leafer 依赖 |
| `update` | 更新项目中的 Leafer 依赖版本号 |
| `plugin` | 创建 LeaferX 插件模版 |

##### 快速创建 `Leafer` 模版项目

运行本命令会在当前目录下生成一个 `Vite` + `Leafer` 的项目模板，可根据提示选择 `Vanilla` / `Vue` / `React` 和 `JavaScript` / `TypeScript`，用于快速构建 leafer 体验环境。

```bash
npm create leafer@latest template
```

目前内置 `Vanilla` / `Vue` / `React` 模版，均提供 `JavaScript` 和 `TypeScript` 版本。
...
该命令会以引导交互的方式添加或更新当前项目中需要的 leafer 依赖。运行的项目中需要包含 leafer 相关依赖。
...
如果希望使用完整命令，也可以把 `leafer` 替换为 `create-leafer`。
